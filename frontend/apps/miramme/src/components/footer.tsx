...
import { Separator, Input, Button, AuroraBackground, RetroGrid, Magnetic } from "@grafiesto/ui"
...
export function Footer() {
  return (
    <footer className="relative border-t border-border overflow-hidden">
      ...
      <div className="relative z-10 bg-card/60 backdrop-blur-3xl text-card-foreground">
        {/* Newsletter section */}
          ...
          <div className="mx-auto grid max-w-5xl gap-12 md:grid-cols-2 md:items-center">
            ...
            <form className="flex gap-2" onSubmit={(e) => e.preventDefault()}>
              <Input
                ...
                className="flex-1 bg-background/50 backdrop-blur-md"
              ...
              {SOCIAL.map(({ Icon, href, label }) => (
                <Magnetic key={label} strength={0.3}>
                  <a
                    ...
                    aria-label={label}
                    className="flex h-9 w-9 items-center justify-center rounded-full border border-border/50 bg-background/50 text-muted-foreground backdrop-blur-md transition-colors hover:border-foreground hover:bg-background hover:text-foreground shadow-sm"
                  >
